...
export function getDeliveryIncidentText(delivery) {
  ...
  return "Incident détecté à mi-parcours : l'IA recalcule la trajectoire depuis la position actuelle du camion.";
}
...
export function getMoroccoDateString() {
  return new Intl.DateTimeFormat("fr-MA", {
    timeZone: "Africa/Casablanca",
    weekday: "long",
    day: "2-digit",
...
// ---------- Signal palette ----------
// Core: olive (primary), copper (secondary), amber (warning), rust (danger), azure (info)
export function riskBadge(risk) {
  if (risk === "Élevé" || risk === "High") {
    return {
      color: "#cf6a4f",
      bg: "rgba(207,106,79,0.12)",
      border: "rgba(207,106,79,0.38)",
      label: "Élevé",
    };
  }
  if (risk === "Moyen" || risk === "Medium") {
    return {
      color: "#d8a84a",
      bg: "rgba(216,168,74,0.12)",
      border: "rgba(216,168,74,0.36)",
      label: "Moyen",
    };
  }
  return {
    color: "#8aaa7a",
    bg: "rgba(138,170,122,0.10)",
    border: "rgba(138,170,122,0.34)",
    label: "Faible",
  };
}
...
export function priorityBadge(priority) {
  if (priority === "Haute")
    return { color: "#cf6a4f", bg: "rgba(207,106,79,0.12)" };
  if (priority === "Moyenne")
    return { color: "#d8a84a", bg: "rgba(216,168,74,0.12)" };
  return { color: "#98a3a0", bg: "rgba(152,163,160,0.10)" };
}
